...
function evalIdStructured(id: string): ScoringCheckResult {
  const hasInvalidChars = /[\s_\-]/.test(id);
  const hasSeparator = id.includes(".");
  const hasPrefix = /^[A-Z]{2,}\./.test(id) || /^\d+\./.test(id);
  
  // All issues in one check
  const issues: string[] = [];
  let points = 50; // Combined max from old checks
  
  if (hasInvalidChars) {
    issues.push("Use structured format with separator (e.g., GDPR.1.1 or NIST.AC.1)");
    points -= 25;
  }
  
  if (!hasSeparator && !hasPrefix) {
    issues.push("Use structured format (e.g., GDPR.1.1 or 03.01.01)");
    points -= 25;
  }
  ...
  return {
    ...
    max: 50,
    status: points >= 35 ? "WARN" : "FAIL",
    notes: issues[0],
    violations: [issues.join(" and ")]
  };
}
...
function evalNameConcise(name: string): ScoringCheckResult {
  ...
  if (words === 0) {
    ...
    points = 10;
    status = "FAIL";
    violations = [`Name too long (${words} words) - keep under ${absMax} words`];
  } else if (words > idealMax) {
    points = 18;
    status = "WARN";
    violations = ["Use concise name"];
  }
  
  return {
    id: "name.concise",
    label: "Concise (≤6 words ideal, ≤12 max)",
    points,
    max: 25,
    status,
    notes: violations?.[0],
