...
// ✅ NEW: Helper function to split HOW into individual artifacts
...
function evalTangibleArtifacts(how: string): ScoringCheckResult {
  ...
  // ✅ NEW: Split HOW into individual artifacts
  ...
  // ✅ NEW: Check EACH artifact individually
  artifacts.forEach((artifact, index) => {
    ...
    if (hasTimeSensitive && !hasTimeframe && !hasExplicitDate) {
      ...
      violations.push(`(-8 pts) "${artifactName}" needs a specific timeframe. Use 'last 30 days', 'last 90 days', 'for Q1 2024', 'covering the review period', or explicit dates.`);
    ...
    if (hasPointInTime && !hasCurrency && !hasExplicitDate && !hasTimeframe) {
      ...
      violations.push(`(-8 pts) "${artifactName}" needs a currency indicator like 'current', 'existing', 'active', 'latest', or 'as-of [date]'.`);
