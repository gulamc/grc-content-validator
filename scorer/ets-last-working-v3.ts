...
function evalOutcomeBased(what: string): ScoringCheckResult {
  ...
  const outcomePatterns = [
    ...
    /\b\w+(?:\s+\w+)?\s+(are|is)\s+(implemented|deployed|configured|maintained|managed|monitored|reviewed|approved|completed|documented|established|enabled|disabled|protected|secured|assigned|authorized|validated|verified|recorded|addressed|tracked|logged|resolved|comprised|composed|exercised)\b/i,
...
function evalWhatHowAlignment(what: string, how: string): ScoringCheckResult {
  ...
  // ✅ NEW: Generate artifact-specific suggestions
  if (missingTerms.length > 0 && alignmentRatio < 0.8) {
    const artifacts = splitIntoArtifacts(how);
    
    // For each missing term, suggest which artifact(s) to add it to
    missingTerms.slice(0, 3).forEach(term => {
      // Find the most relevant artifact (first one, or the one that seems most related)
      let suggestedArtifactIndex = 0;
      
      // Smart heuristic: if term relates to policies/procedures, suggest first artifact
      if (/document|policy|procedure|guideline/.test(term)) {
        suggestedArtifactIndex = 0;
      }
      // If term relates to records/logs, suggest later artifacts
      else if (/record|log|report|evidence/.test(term)) {
        suggestedArtifactIndex = Math.min(1, artifacts.length - 1);
      }
      
      const artifactIdentifier = extractArtifactIdentifier(how, artifacts[suggestedArtifactIndex]) || 
                                  (suggestedArtifactIndex === 0 ? 'i)' : 
                                   suggestedArtifactIndex === 1 ? 'ii)' : 
                                   suggestedArtifactIndex === 2 ? 'iii)' : `${suggestedArtifactIndex + 1})`);
      
      const artifactPreview = artifacts[suggestedArtifactIndex]?.substring(0, 50) || 'first artifact';
      
      violations.push(
        `Artifact ${artifactIdentifier} Consider adding "${term}" (referenced in WHAT) to improve alignment`
      );
    });
  }
    ...
    for (const check of dim.checks) {
      if (check.violations) {
        check.violations.forEach(v => {
          // Skip low-priority violations
          const shouldSkip = skipPatterns.some(pattern => pattern.test(v));
          
          if (!suggestions.includes(v) && !shouldSkip) {
            suggestions.push(v);
          }
        });
      }
