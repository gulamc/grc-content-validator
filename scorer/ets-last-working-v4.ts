...
function evalMeasurableTerms(what: string): ScoringCheckResult {
  ...
  // List of vague, subjective, unmeasurable terms
  const vagueTerms = [
    ...
    'sound', 'solid', 'strong', 'weak'
  ];
  
  const lowerWhat = what.toLowerCase();
  const foundVague: string[] = [];
  
  // Check for each vague term with word boundaries
  ...
  if (foundVague.length > 0) {
    return {
      ...
      max,
      status: "FAIL",
      notes: `Contains vague/unmeasurable terms: "${foundVague.join('", "')}"`,
      violations: [`Contains vague/unmeasurable terms: "${foundVague.join('", "')}". Use concrete, verifiable criteria instead. Example: instead of "reasonable access provisioning", use "access requests approved by authorized personnel".`]
    };
  }
...
async function evalWhatHowAlignment(
  ...
  // ✅ NEW: Filter out vague/unmeasurable terms - these should NOT be added to HOW
  const vagueTerms = [
    'reasonable', 'sufficient', 'adequate', 'appropriate',
    'suitable', 'proper', 'satisfactory', 'acceptable',
    'effective', 'efficient', 'timely', 'quality',
...
function buildSuggestions(...dims: DimensionResult[]): string[] {
  ...
  // ✅ NEW: Separate by priority
  const criticalViolations: string[] = [];
  const importantViolations: string[] = [];
    ...
    for (const check of dim.checks) {
      if (check.violations) {
        check.violations.forEach(v => {
          ...
          if (shouldSkip || suggestions.includes(v)) return;
          
          // ✅ Categorize by severity
          if (/framework|specific compliance|ISO|SOC|NIST/i.test(v)) {
            criticalViolations.push(v); // Framework references are critical
          } else if (/outcome|tangible|timeframe/i.test(v)) {
            importantViolations.push(v); // Core structure issues
          } else {
            minorViolations.push(v); // Everything else
          }
        });
      }
    }
  ...
  return [...criticalViolations, ...importantViolations, ...minorViolations].slice(0, 8);
}
