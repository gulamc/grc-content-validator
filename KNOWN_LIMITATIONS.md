...
- **B1 (rules-b.ts "Rules and Regulations" false-positive class)** — **FIXED 2026-06-19** (Defect 1, narrow). The bare `Regulations?\s` alternative in `CITATION_START_RE` was replaced with the specific shapes the rule actually wants to recognise: `Regulation\s+\((?:EU|EC|EEC)\)`, `Regulation\s+No\.\s`, `Implementing\s+Regulations?\s`. Then **superseded 2026-06-21 by Path B** (below), which addresses the predicate's root shape rather than patching individual trigger tokens.

- **B1 (predicate over-aggressive — splits ANY citation matching the trigger tokens)** — **FIXED 2026-06-21**. After Defect 1 the only surviving B1 firing in the 5-doc test set was Q3.1.10 Philippines, splitting inside the Disini case citation at `"G.R. No."` (Defect 2 class). Enumerating ALL B1 firings across all 5 docs showed B1 had fired 1 time total, and that firing was a false positive. Pre-Defect-1 it had fired 3 times — same shape: all false positives. **B1 had never fired legitimately on the test set.** Rejected the symptom-by-symptom patching path (Defect 2 narrow fix, then 3, then 4, …) and rewrote the predicate to encode the spec semantic directly: split ONLY when both sides reference DIFFERENT instruments. New helpers `extractInstrumentTail` (regex on trailing `" of [the] X"`, bounded length, conservative) and `shouldSplit` (false-negative-safe: any ambiguity → don't split) are shared between the trigger predicates (`hasAndJoinedLaws`, `hasPeriodJoinedLaws`) and the splitters (`splitOnAndNotInParens`, `splitPeriodJoinedLine`) so detection and rewrite cannot diverge. On the current 5-doc set this produces ZERO B1 firings — the correct answer per the data — and the spec positive example (`"Articles 2-5 of the GDPR and Articles 5, 7, and 9 of the National Law"`) still splits correctly. The committed unit test at `scripts/verify-b1-predicate.mjs` is the proof B1's split path still works in the absence of any 5-doc-set firings.
