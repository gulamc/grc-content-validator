...
export function AIEnhancedSuggestionsControl({
  ...
  // Only show AI suggestions when there's a score result AND verdict is not 'pass'
  // Hide on PASS scores (>= 85) to avoid unnecessary suggestions
  if (!enabled || !scoreResult) {
    return null;
  }
  
  // Check if verdict is 'pass' - if so, don't show AI suggestions
  const verdict = (scoreResult as any).verdict;
  if (verdict === 'pass' || (scoreResult.overall_score && scoreResult.overall_score >= 85)) {
    return null;
  }
